...
class Player:
    def __init__(self, filename, background, screen):
        self.fig = myFig(filename, background, screen)
        self.thrust = 0.007
        self.rot = 90
        self.thrust_append = 0.0005
        self.fired = False
        self.dead = False
        self.shot = None
        
    def draw(self):
        return self.fig.draw()
    
    def update(self):
        if self.fired and self.shot:
            if self.shot.distance > self.fig.screenRect.bottom:
                self.fired = False
                self.shot = None
                self.fig.screen.blit(self.fig.background, self.fig.screenRect.topleft)
            if self.fired:
                if self.shot.distance > 20:
                    self.shot.lethal = True
        return self.fig.update()
    
    def shoot(self):
        self.shot = Shot("shot.png", self.fig.background, self.fig.screen, self.fig.speed, self.fig.pos, copy(self.fig.rot))
        self.shot.update()
        self.fired = True

    def die(self):
        self.dead = True
        self.screen.blit(self.fig.background, self.fig.screenRect.topleft)
        self.Font = pygame.font.SysFont(None,32)
        LevelText = self.Font.render("Muahaha!!! Game over! q-quit, r-try again", True, (255,255,255))
        self.screen.blit(LevelText,(self.fig.screenRect.centerx-220,self.fig.screenRect.centery-10))
        self.fired = False
        self.shot = None
...
class Asteroid:
    def __init__(self, filename, background, screen, life, arr, tellPos, tellSpeed, playerRect):
        ...
        if tellPos == 0:
            myposx = random.randint(0,self.fig.screenRect.right)
            myposy = random.randint(0,self.fig.screenRect.bottom)
            self.fig.pos = [myposx,myposy]
            self.update()
            #print playerRect.colliderect(self.collideRect)
            while playerRect.colliderect(self.fig.collideRect):
                myposx = random.randint(0,self.fig.screenRect.right)
                myposy = random.randint(0,self.fig.screenRect.bottom)
...
class aGame:
    ...
    def __init__(self):
        ...
        self.levelCnt = 1
        self.asteroids = []
        self.asteroids.append(Asteroid("asteroid_small.png",self.bgSurface, self.screen,1, self.asteroids,0,0,self.player.fig.collideRect))
        self.asteroids.append(Asteroid("big_asteroid.png",self.bgSurface, self.screen,2, self.asteroids,0,0,self.player.fig.collideRect))
...
#            self.screen.blit(LevelText,(10,10))
    ...
    def GetInput(self):
        ...
        self.player.fig.speed[0] *= 0.995
        self.player.fig.speed[1] *= 0.995
        if self.player.thrust > 0.007:
            self.player.thrust *= 0.99
        if keystate[K_LEFT]:
            self.player.fig.rot += 1.0
            if self.player.fig.rot > 360:
                self.player.fig.rot = 0
        if keystate[K_RIGHT]:
            self.player.fig.rot -= 1.0
            if self.player.fig.rot < 0:
                self.player.fig.rot = 359
        if keystate[K_UP]:
            self.player.thrust += self.player.thrust_append;
            self.player.fig.speed[0] += self.player.thrust*sin(radians(self.player.fig.rot))
            self.player.fig.speed[1] += self.player.thrust*cos(radians(self.player.fig.rot))
        if keystate[K_DOWN]:
            self.player.thrust = 0.007
            self.player.fig.speed[0] *= 0.993
            self.player.fig.speed[1] *= 0.993
        if keystate[K_SPACE]:
        ...
        for aster in self.asteroids:
            if self.player.fired:
                if self.player.shot.rect.colliderect(aster.collideRect):
                    ...
                    return True
            if self.player.fig.collideRect.colliderect(aster.fig.collideRect):
                if act:
                    self.player.die()
                return True
    ...
    def teleportPlayer(self):
            ...
            while self.collisionDetect(False):
                myposx = random.randint(0,myScreen.right)
                myposy = random.randint(0,myScreen.bottom)
                self.player.fig.pos = [myposx,myposy]
                self.screen.blit(self.bgSurface, myScreen.topleft)
                self.player.update()
                pygame.display.update()
